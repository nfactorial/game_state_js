...
class GameState {
     ...
     * @param desc {Object} Description of the state to be initialized.
     */
    constructor(desc) {
        this.parent = null;
        this.childNames = [];
     ...
     * @param branchRoot {GameState} The root state where walking up the hierarchy will stop.
     * @param initArgs {object}
     */
    onInitialize(branchRoot, initArgs) {
        if (this.parent !== branchRoot) {
            this.parent.onInitialize(initArgs);
        }
     ...
     * @param branchRoot {GameState}
     */
    onDestroy(branchRoot) {
        // Systems are notified of destruction in reverse order
        for (let index = this.gameSystems.length - 1; index >= 0; index--) {
            this.gameSystems[index].onDestroy();
        }

        if (this.parent !== branchRoot) {
            this.parent.onDestroy(branchRoot);
        }
    }

    /**
     * Called by the framework when this game state is becoming active within the running session.
     * @param branchRoot {GameState} The branch point iin the state tree where the switch occurred.
     */
    onEnter(branchRoot) {
        if (this.parent !== branchRoot) {
     ...
     * @param branchRoot {GameState} The branch point in the state tree where the switch occurred.
     */
    onExit(branchRoot) {
        if (this.parent !== branchRoot) {
     ...
     * @param name {String} The name associated with the system to be retrieved.
     ...
     * @param state {GameState} The game state to be checked.
