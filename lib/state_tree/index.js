...
const InitArgs = require('../init_args');
...
class StateTree {
    ...
    onInitialize(initArgs) {
        ...
        for (const state of this.stateMap.values()) {
            ...
            if (!state.parent) {
                this.initArgs.state = state;

                state.onInitialize(this.initArgs);
            }
        }
    }
...
StateTree.commitStateChange = function(stateTree) {
    if (stateTree.pendingState) {
        ...
        const rootState = StateTree.findCommonAncestor(stateTree.activeState, stateTree.pendingState);
        ...
        stateTree.activeState.onEnter(rootState);
    }
};
